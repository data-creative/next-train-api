...
RSpec.describe "gtfs" do
  ...
  describe ":force_import" do
    ...
    it "should perform a destructive_mode GtfsImport" do
      #expect_any_instance_of(GtfsImport).to receive(:initialize).with(hash_including(:destructive_mode => true)) # warns... "warning: removing `initialize' may cause serious problems"
      expect_any_instance_of(GtfsImport).to receive(:perform)
