...
class GtfsImport < ApplicationJob
  ...
  def self.perform(source_url: "http://www.shorelineeast.com/google_transit.zip", destination_path: "./tmp/google_transit.zip", system_schedule: TransitSchedule.latest)
    ...
    schedule.update(:content_length => headers["content-length"].first.to_i, :etag => headers["content-length"].first.tr('"',''))
